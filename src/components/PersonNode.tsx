...
export function PersonNode({ id, data, selected }: { id: string; data: PersonData; selected: boolean }) {
  ...
  return (
    ...
    >
      <div 
        className={`w-48 bg-white rounded-xl border border-slate-200 shadow-sm transition-all duration-200 group ${selected ? 'ring-2 ring-indigo-400 border-indigo-400 shadow-md' : 'hover:border-slate-300 hover:shadow'}`}
      >
      ...
      {/* Action Menu overlay - wrapped in a container that extends down to bridge the gap */}
      <div className={`absolute -top-12 left-1/2 -translate-x-1/2 pt-2 pb-4 transition-opacity duration-200 z-10 ${showMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-white rounded-lg shadow-lg border border-slate-200 p-1 flex gap-1">
          <button 
